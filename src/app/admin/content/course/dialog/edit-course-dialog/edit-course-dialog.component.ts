...
export class EditCourseDialogComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.statusSelected = this.course.status;
  }

  public updateCourse() {
    console.log(this.data._course.id);
    this.httpClient.put(`http://localhost:61616/api/Courses/${this.course.id}`, this.course).subscribe(result => {
      setTimeout(() => this.toastrService.success('Update thành công !', 'Dữ liệu khóa học'));
    });
